...
if(isset($_POST['payer']))
{
	...
	for($i=0 ;$i < count($_SESSION['panier']['id_produit']) ; $i++) 
	{
		...
		$produit = $resultat->fetch_assoc();
		array_push($lstProd, $produit['stock']);
	...
	if(!isset($erreur))
	{
		...
		for($i = 0; $i < count($_SESSION['panier']['id_produit']); $i++)
		{
			...
			$qte = $lstProd[$i] -  $_SESSION['panier']['quantite'][$i];
			executeRequete("UPDATE produit SET stock=$qte WHERE id_produit=" . $_SESSION['panier']['id_produit'][$i] );
